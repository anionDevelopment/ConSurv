...
def start_dockerfile_example():
    ...
    parser.add_argument('-r', '--resetpersistece',  required=False, action='store_true', default=False)
    ...
    if args.verbose:
        env_values["InitialVerboseValue"] = "true"
    t.ensure_env_file_is_generated(current_file, env_file_name,env_values)
    t.start_dockerfile_example(current_file, True, args.resetpersistece, env_file_name)
